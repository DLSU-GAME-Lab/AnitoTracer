...
{
	const float dotProduct = dot(direction, normal);
	const vec3 outwardNormal = dotProduct > 0 ? -normal : normal;
	const float niOverNt = dotProduct > 0 ? m.RefractionIndex : 1 / m.RefractionIndex;
	const float cosine = dotProduct > 0 ? m.RefractionIndex * dotProduct : -dotProduct;
...
// Diffuse Light
RayPayload ScatterDiffuseLight(const Material m, const float t, inout uint seed, vec3 light, int anyHitFlag)
{
