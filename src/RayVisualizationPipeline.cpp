...
namespace Vulkan {
	...
	RayVisualizationPipeline::RayVisualizationPipeline(
		...
		swapChain_(swapChain)
	{
		...
		// Load shaders.
		const ShaderModule vertShader(device, FileUtils::getAssetsFolderPath().generic_string() + "/shaders/RayVisualization.vert.spv");
		const ShaderModule fragShader(device, FileUtils::getAssetsFolderPath().generic_string() + "/shaders/RayVisualization.frag.spv");
	...
	VkDescriptorSet RayVisualizationPipeline::DescriptorSet(const uint32_t index) const
	{
