...
namespace Vulkan {
		...
		scene_(scene)
	{
		...
		{ // Create Viewport Image
			...
			for (int i = 0; i < SwapChain().Images().size(); i++)
			{
				...
				outputImageMemory_.reset(new DeviceMemory(AllocateImageMemory(outputImage_->Handle())));
				...
				outputImageView_.reset(new ImageView(SwapChain().Device(), outputImage_->Handle(), format, VK_IMAGE_ASPECT_COLOR_BIT));
			}

			const auto& debugUtils = SwapChain().Device().DebugUtils();

			debugUtils.SetObjectName(outputImage_->Handle(), "Viewport Image");
			debugUtils.SetObjectName(outputImageMemory_->Handle(), "Viewport Image Memory");
			debugUtils.SetObjectName(outputImageView_->Handle(), "Viewport ImageView");
		}


		// TransitionImageLayout(*commandPool_, outputImage_->Handle(), VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
		// TransitionImageLayout(*commandPool_, outputImage_->Handle(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
		viewportDSet_ = ImGui_ImplVulkan_AddTexture(
			sampler_->Handle(),
			outputImageView_->Handle(),
	...
	void Viewport::RenderRasterized(VkCommandBuffer commandBuffer, const uint32_t imageIndex)
	{
		...
		vkCmdCopyImage(commandBuffer,
			...
			outputImage_->Handle(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
			1, &copyRegion);
	...
	void Viewport::RenderRayTraced(VkCommandBuffer commandBuffer, uint32_t imageIndex, const VkImage image)
	{
		...
		VkOffset3D blitSize;
		blitSize.x = swapChain_.Extent().width;
		blitSize.y = swapChain_.Extent().height;
		blitSize.z = 1;
	...
	void Viewport::drawUI()
	{
		...
		ImGui::Image(reinterpret_cast<ImTextureID>(viewportDSet_), ImVec2{ viewportPanelSize.x, viewportPanelSize.y });
		ImGui::End();
