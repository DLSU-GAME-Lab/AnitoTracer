...
RayTracingPipeline::RayTracingPipeline(
	...
	VkBuffer workQueueBuffer,
	VkBuffer workQueueCountBuffer) :
	swapChain_(swapChain)
{
	...
	const std::vector<DescriptorBinding> descriptorBindings =
	{
		...
		{ 15, 1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_RAYGEN_BIT_KHR }, // Work queue buffer
		{ 16, 1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_RAYGEN_BIT_KHR }, // Work queue count buffer
	};
	...
	for (uint32_t i = 0; i != swapChain.Images().size(); ++i)
	{
		...
		std::vector<VkWriteDescriptorSet> descriptorWrites =
		{
			...
			descriptorSets.Bind(i, 15, workQueueBufferInfo),
			descriptorSets.Bind(i, 16, workQueueCountBufferInfo),
		};
