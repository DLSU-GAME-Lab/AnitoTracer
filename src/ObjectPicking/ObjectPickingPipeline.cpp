#include "ObjectPickingPipeline.h"
...
namespace Vulkan {
	...
	ObjectPickingPipeline::ObjectPickingPipeline(
		...
		swapChain_(swapChain)
	{
		...
		renderPass_.reset(new class RenderPass(swapChain, depthBuffer, VK_ATTACHMENT_LOAD_OP_LOAD, VK_ATTACHMENT_LOAD_OP_CLEAR));

		// Load shaders.
		const ShaderModule vertShader(device, FileUtils::getAssetsFolderPath().generic_string() + "/shaders/ObjectPicking.vert.spv");
		const ShaderModule fragShader(device, FileUtils::getAssetsFolderPath().generic_string() + "/shaders/ObjectPicking.frag.spv");
		...
		Check(vkCreateGraphicsPipelines(device.Handle(), nullptr, 1, &pipelineInfo, nullptr, &pipeline_),
			"create ray visualization pipeline");
	}
	...
	VkDescriptorSet ObjectPickingPipeline::DescriptorSet(const uint32_t index) const
	{
